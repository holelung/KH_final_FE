  ...
  return (
    <>
      <div className="flex flex-col justify-start max-w-full mx-3 h-full font-PretendardM text-lg">
        <h1 className="text-5xl font-PretendardB text-left my-3">회원 관리</h1>
          ...
          <table className="table-fixed w-full border-separate border-spacing-4">
            <thead className="font-PyeojinGothicB text-lg">
              <tr>
                ...
                <th className="w-1/8">부서</th>
                <th className="w-1/8">팀</th>
                <th className="w-1/8">직급</th>
              ...
              {data.length > 0 ? (
                data.map((item) => (
                  <tr key={item.id} onClick={() => handleDetailOpen(item)}>
                    ...
                    <td>{item.username}</td>
                    <td>{item.realname}</td>
                    <td>{item.deptName? item.deptName : "없음"}</td>
                    <td>{item.teamName? item.teamName : "없음"}</td>
                    <td>{item.jobName? item.jobName : "없음"}</td>
                    <td>{item.enrollDate?.replace("T"," ")}</td>
                  </tr>
                ))
              ) : (
                <tr>
